...
class MyAntEnv(MujocoEnv, utils.EzPickle):
    FILE = "ant.xml"
    ORI_IND = 3
    ...
    def get_current_obs(self):
        if self.ego_obs:
            ...
            return np.concatenate([
                ...
                self.data.get_body_xpos("torso").copy(),  # use xpos instead of com
            ]).reshape(-1)
        else:
            return np.concatenate([
                self.data.qpos.flat.copy(),
                self.data.qvel.flat.copy(),
                np.clip(self.data.cfrc_ext, -1, 1).flat.copy(),
                self.data.get_body_xmat("torso").flat.copy(),
                self.data.get_body_xpos("torso").copy(),
            ]).reshape(-1)
...
class AntTwoResourceEnv(TwoResourceEnv):
    """
...
class AntSmallTwoResourceEnv(TwoResourceEnv):
    """
